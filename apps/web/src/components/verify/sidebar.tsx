...
import { cn } from "@/lib/verify/utils";
import { useVerifyStore } from "@/store/verify";
import { employerApi } from "@/lib/verify/api";
import { toast } from "sonner";
import { Avatar, AvatarFallback } from "@/components/verify/ui/avatar";
import { Tooltip, TooltipContent, TooltipProvider, TooltipTrigger } from "@/components/verify/ui/tooltip";

const navItems = [
  { href: "/verify/dashboard", label: "Dashboard", icon: LayoutDashboard },
  { href: "/verify/verify", label: "Request Verification", icon: ShieldCheck },
  { href: "/verify/verifications", label: "Verification History", icon: History },
  { href: "/verify/billing", label: "Billing", icon: CreditCard },
  { href: "/verify/settings", label: "Settings", icon: Settings },
];
      ...
      <nav className="flex-1 py-4 space-y-0.5 overflow-y-auto">
        <TooltipProvider delayDuration={0}>
          {navItems.map(({ href, label, icon: Icon }) => {
            const active = pathname === href || (href !== "/verify/dashboard" && pathname.startsWith(href));
            return (
